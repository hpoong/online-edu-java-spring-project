package com.hopoong.flow.api.service;

import com.hopoong.flow.exception.ErrorCode;
...
public class UserQueueService {
    ...
    public Mono<Boolean> isAllowedByToken(final String queue, final Long userId, final String token) {
        return generateToken(queue, userId)
                .filter(data -> data.equalsIgnoreCase(token))
                .map(i -> true)
    ...
    @Scheduled(initialDelay = 5000, fixedDelay = 10000) // 10초 주기
